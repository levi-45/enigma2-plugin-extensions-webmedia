...
def getUrlresp(url):
        pass#print "Here in getUrlresp url =", url
...
def getpics(names, pics, tmpfold, picfold):
              pass#print "In getpics tmpfold =", tmpfold
              pass#print "In getpics picfold =", picfold
...
#                          picf = picfold + "/" + name + ".jpg"
                   ...
                   if fileExists(picf):
                          cmd = "cp " + picf + " " + tmpfold
                          pass#print "In getpics fileExists(picf) cmd =", cmd
                          os.system(cmd)
                   
                   if not fileExists(picf):
                       pass#print "In getpics not fileExists(picf) url =", url
                       if THISPLUG in url:
                          try:
                                  cmd = "cp " + url + " " + tpicf
                                  pass#print "In getpics not fileExists(picf) cmd =", cmd
                                  os.system(cmd)
                          except:
                                  pass
                       else:
                          pass#print "In getpics not THISPLUG"
                          try:
                               if "|" in url:
                                  n3 = url.find("|", 0)
                                  ...
                                  n2 = url.find("=", n1)
                                  url1 = url[:n3]
                            #      pass#print "In getpics not fileExists(picf) url1 =", url1
                                  referer = url[n2:]
                            #      pass#print "In getpics not fileExists(picf) referer =", referer
                                  p = getUrl2(url1, referer)
                                  f1=open(tpicf,"wb")
                                  f1.write(p)
                                  f1.close() 
                               else:
                                  pass#print "Going in urlopen url =", url
                                  f = getUrlresp(url)
                                  pass#print "f =", f
                                  p = f.read()
                                  f1=open(tpicf,"wb")
                       ...
                       if not fileExists(tpicf): 
                                  pass#print "In getpics not fileExists(tpicf) tpicf=", tpicf
                                  """
                                  if ".png" in tpicf:
                                  ...
                                  pass#print "In getpics not fileExists(tpicf) cmd=", cmd        
                                  os.system(cmd)
              ...
              pass#print "In getpics final cmd1=", cmd1  
              os.system(cmd1)
              return pix
...
class GridMain(Screen):
        ...
        def __init__(self, session, names, urls, pics = []):
                ...
                if DESKHEIGHT > 1000:
                        pass#print "DESKHEIGHT > 1000 =", DESKHEIGHT
                ...
                pass#print "In Gridmain pics = ", pics
                self.pos = []
                if config.plugins.webmedia.skinres.value == "fullhd": 
...
#################################
                    ...
                    if os.path.exists(pic):
                           pass#print "pic path exists"
                    else:
                           pass#print "pic path exists not"
                    
                    picd = "/usr/lib/enigma2/python/Plugins/Extensions/WebMedia/skinpics/images/default.png"
                    if os.path.exists(picd):
                           pass#print "pic path 2 exists"
                    else:
                           pass#print "pic path 2 exists not"
...
#          pass#print "In XbmcPluginScreen self.url =", self.url
          pass#print "In XbmcPluginScreen self.name =", self.name
          pass#print "In XbmcPluginScreen self.index =", self.index   
          pass#print "Here in okClicked going in Playoptions"
#                system("sleep 2")
          f1=open("/tmp/wmindex","w")
          itype = str(itype)
          f1.write(itype)
