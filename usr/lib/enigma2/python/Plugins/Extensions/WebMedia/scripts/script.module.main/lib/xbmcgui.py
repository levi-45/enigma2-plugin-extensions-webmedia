...
#		print "*** dialog %s ***\n%s <<< %s" % (type, sel, dlg)
	...
	def choose(self, type, query, list, default=0):
                print "Here in xbmcgui.py sys.argv =", sys.argv
                print "xbmcgui.py choose list =", list
 #               rfile = THISPLUG + "/result.txt"
                rfile = "/tmp/result.txt"

                if not os.path.exists(rfile):
                        print "xbmcgui.py no result.txt ="
                        f = open("/tmp/arg1.txt", "w")
                        text1 = sys.argv[0]
                        text1 = text1.replace("plugin://", "/usr/lib/enigma2/python/Plugins/Extensions/WebMedia/plugins/")
                        text1 = text1 + "default.py"
                        text = text1 + " " + sys.argv[1] + " " + sys.argv[2] + " &\n"
                        print "xbmcgui.py choose arg1.txt created text =", text
                        f.write(text)
                        f.close()
...
#                        return "false"
                        ...
                        print "xbmcgui.py yes result.txt"
                        myfile = open("/tmp/result.txt","r")       
                        icount = 0
                        for line in myfile.readlines(): 
                               idx = line
                               print "xbmcgui.py yes idx 1=", idx
                               icount = icount+1
                               if icount > 0:
                                      break
                        myfile.close()
                        cmd = "rm " + rfile
                        os.system(cmd)
                        print "xbmcgui.py yes int(idx) 2=", int(idx)
                        return int(idx)
	        else:
                        sys.exit()
...
class ControlLabel:
        ...
        def __init__(self):
                print "In xbmcgui.py ControlLabel"

	def setLabel(self, label):
	                print "In xbmcgui.py ControlLabel label =", label
                        pass		
class ControlTextBox:
        def __init__(self):
                print "In xbmcgui.py ControlTextBox"
	def setText(self, text):
	                print "In xbmcgui.py ControlTextBox text =", text
	                rfile = "/tmp/show.txt"
...
#		pass
	...
	def getControl(self, controlId):
		...
		print "In xbmcgui.py getControl controlId =", controlId
		if controlId == 1:
		        return ControlLabel()
		elif controlId == 5:        
