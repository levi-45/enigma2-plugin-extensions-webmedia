...
def log(txt):
#        f = open("/usr/lib/enigma2/python/Plugins/Extensions/WebMedia/kodi_log.txt", "a")
...
def addDirectoryItem(handle, url, listitem, totalItems=40, isFolder=True):
       ...
       print "xbmcplugin addDirectoryItem plug =", plug
       if os.path.exists("/etc/debugxb"):
           try:
               print "xbmcplugin addDirectoryItem plug =", plug
       ...
       if plug in url:
               ...
               url = "/usr/lib/enigma2/python/Plugins/Extensions/WebMedia/plugins/" + plug + "/default.py?" + url
       print "xbmcplugin addDirectoryItem url B =", url
       ...
       if os.path.exists("/etc/debugxb"):
          try:
             print "xbmcplugin addDirectoryItem data =", data
          except:
             pass   
       file = open("/tmp/data.txt", "a")
       data1 = getdata(listitem)
       print "xbmcplugin addDirectoryItem data1 =", data1
       data = "&" + data1 + data
       data = data.replace("\n", "")
       data = data.replace("\r", "")
       data = data + "\n"
...
#              print "label B=", label
#              if "plugin://" in path:
              path = path.replace("plugin://", "/usr/lib/enigma2/python/Plugins/Extensions/WebMedia/plugins/")
              ...
              if os.path.exists("/etc/debugxb"):
                  try:
                      print "In xbmcgui Kodi data =", data
                      print "<<<"
                  except:        
                      pass
#              file = open("/tmp/data.txt", "a")
...
#       plug = xbmcaddon.getaddonpath_params("addon_id")
       ...
       if os.path.exists("/etc/debugxb"):
           try:
               print "xbmcplugin addDirectoryItem plug =", plug
               print "xbmcplugin addDirectoryItems handle =", handle
               print "xbmcplugin addDirectoryItems items =", items
               print "xbmcplugin addDirectoryItems totalItems =", totalItems
           except:
               psss    
       ...
       while i < totalItems:
               plug = xbmcaddon.getaddonpath_params("addon_id")
               print "xbmcplugin addDirectoryItems plug =", plug
               url = items[i][0]
               url = url.replace("&", "AxNxD")
               url = url.replace("=", "ExQ")
               url = "/usr/lib/enigma2/python/Plugins/Extensions/WebMedia/plugins/" + plug + "/default.py?" + url
#               log("\nxbmcplugin addDirectoryItems url =%s" %url)
               print "xbmcplugin addDirectoryItems url =", url
               name = items[i][1].getLabel()
               print "xbmcplugin addDirectoryItems name =", name
...
def setResolvedUrl(handle, succeeded, listitem, stack=-1):
	   """Callback function to tell XBMC that the file plugin has been resolved to a url"""
           ...
           print "Here in setResolvedUrl 1"
           dataA = listitem.data
           print "Here in setResolvedUrl dataA =", dataA
           thumbnailImage = dataA['thumbnailImage']
           label = dataA['label']
           print "Here in setResolvedUrl label =", label
           path = dataA['path']
           label2 = dataA['label2']
           iconImage = dataA['iconImage']
           ...
           url=dataA['path']
           print "In setResolvedUrl url = ",url
           import xbmc
           player=xbmc.Player()
