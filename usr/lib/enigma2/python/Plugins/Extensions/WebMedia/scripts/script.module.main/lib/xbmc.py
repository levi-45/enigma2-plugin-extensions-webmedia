...
def log(msg, level=None):
	...
	if os.path.exists("/etc/debugxb"):
	       pass#print "Here in xmbc.py log =", msg
	pass       
...
def executebuiltin(function):
    ...
    if not "Container.Update" in function:
        ...
        n3 = url.find("?", 0)
        arg3 = url[n3:]
        arg1 = sys.argv[0].replace("plugin://","/usr/lib/enigma2/python/Plugins/Extensions/WebMedia/plugins/") + "default.py"
        arg2 = sys.argv[1]  
        cmd = "python " + arg1 + " " + arg2 + " '" + arg3 + "' &"
        print "In xbmc.py cmd =", cmd 
        os.system(cmd)
...
def translatePath(url, asURL=False):
	...
	xurl = url
        ...
        if "special://home/addons/" in url:
            url = url.replace("special://home/addons/", "/usr/lib/enigma2/python/Plugins/Extensions/WebMedia/plugins/")
            print "In xbmc url B=", url
            return url
...
#        if not xurl.endswith("/"):
        ...
        n3 = xurl.rfind("/", 0, n2)
        xpath = xurl[:n3]
        print "translatePath xpath =", xpath
        cmd = "mkdir -p " + xpath
        system(cmd)
        return xurl
...
#	return path
        print "In validatePath path =", path
        return path
    ...
    def __init__(self, default=None, heading=None, hidden=False):
        ...
        Example:
            ...
            if (kb.isConfirmed()):
                text = kb.getText()
        """
        pass#print "In xbmc-py default =", default
#        pass
...
#                print "Here in xbmc-py PlayListItem url =", url
...
#		print "**** PlayList.add ****", url, '' if listitem is None else listitem.__dict__
		if not isinstance(url, list):
			url = [url]
		for u in url:
#                        print "Here in xbmc-py u =", u
			item = PlayListItem(u, listitem)
#			print "Here in xbmc-py item =", item
...
#                print "Here in xbmc-py self.items[0].getfilename() =", self.items[0].getfilename()
